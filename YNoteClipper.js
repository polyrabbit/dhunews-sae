...
(function() {
    ...
    Page.common = {trim: function(str) {
            ...
            return {left: document.documentElement.scrollLeft + document.body.scrollLeft,top: document.documentElement.scrollTop + document.body.scrollTop}
        }};
    Page.prototype = {IGNORE_TAGS: ['HTML', 'HEAD', 'META', 'TITLE', 'SCRIPT', 'STYLE', 'LINK', 'IMG', 'FORM', 'INPUT', 'BODY', 'BUTTON', 'TEXTAREA', 'SELECT', 'OPTION', 'LABEL', 'IFRAME', 'UL', 'OL', 'LI', 'DD', 'DL', 'DT', 'A', 'OBJECT', 'PARAM', 'EMBED', 'NOSCRIPT', 'EM', 'B', 'STRONG', 'I', 'INS', 'BR', 'HR', 'PRE', 'H1', 'H2', 'H3', 'H4', 'H5', 'CITE'],getMainArticle: function() {
            var mainArticle = null, hostname = '';
            if (!!location) {
            ...
            var allElems = this.contentDocument.getElementsByTagName('*'), elems = [];
    ...
    Article.prototype = {IGNORE_TAGS: ['A', 'DD', 'DT', 'OL', 'OPTION', 'DL', 'DD', 'SCRIPT', 'STYLE', 'UL', 'LI', 'IFRAME'],TITLE_TAGS: ['H1', 'H2', 'H3', 'H4', 'H5', 'H6'],MINOR_REGEXP: /comment|combx|disqus|foot|header|menu|rss|shoutbox|sidebar|sponsor/i,MAJOR_REGEXP: /article|entry|post|body|column|main|content/i,TINY_REGEXP: /comment/i,BLANK_REGEXP: /\S/i,_getAllTexts: function(node, num) {
            var result = [];
            if (num > 0) {
                var subNode = node.firstChild;
                while (subNode) {
                    if (subNode.nodeType == 3 && this._checkLength(subNode)) {
                        var parent = subNode.parentNode || {}, grandParent = parent.parentNode || {};
            ...
            for (var textIndex = 0, textLength = this._texts.length; textIndex < textLength; textIndex++) {
                var text = this._texts[textIndex], length = Page.common.trim(text.nodeValue).length, weight = 1;
                if (length < 20) {
                    continue;
                }
            ...
            if (arr.length > 2) {
                ...
                for (var count = 0, epos = Page.common.findPos(this.elem), cheight = eheight * 0.05, j = 0; arr[j]; j++) {
                    var tpos = Page.common.findPos(arr[j]);
                    tpos.y - epos.y > cheight && (tpos.y + arr[j].offsetHeight) - (epos.y + eheight) && count++;
                }
                if (eheight / count < 300)
                    return true;
            }
            return false;
...
(function() {
    ...
    YNote.Common = {'browser': (function() {
            ...
            var ev = {'target': (YNote.Common.browser.isIE ? e.srcElement : e.target),'offsetX': (YNote.Common.browser.isIE ? e.offsetX : e.layerX),'offsetY': (YNote.Common.browser.isIE ? e.offsetY : e.layerY),'x': (YNote.Common.browser.isIE ? e.x : e.pageX),'y': (YNote.Common.browser.isIE ? e.y : e.pageY)};
            return ev;
            ...
            if (el.nodeType && el.nodeType == 1 && el.tagName.toLowercase() == 'div') {
                ...
                YNote.Common.addEvent(el, 'mousedown', function(event) {
                    if (typeof win.YNoteDragObject == 'undefined' || win.YNoteDragObject == null) {
                        ...
                        win.YNoteDragObject = {'element': evt.target,'startX': evt.offsetX,'startY': evt.offsetY};
                    } else {
                        return false;
                    }
                });
            ...
            console.log(mess);
        },'Dom': {'appendHTMLToIframe': function(iframe, html) {
    ...
    YNote.Common.extend(YNote.EventInterface.prototype, {'addEventListener': function(eventType, fn) {
    ...
    YNote.Common.extend(YNote.StyleUtil.prototype, {'styleForNode': function(node, parentNode) {
            this.cssNameMap = {};
    ...
    YNote.Common.extend(YNote.Clipper, {'CLASS_INIT': 0,'CLIPPING': 1,'CLIPPED': 2,'UPLOADING_FILE': 3,'UPLOADED_FILE': 4,'UPLOADING_INFO': 5,'UPLOADING_INFO': 6,'START_LOGIN': 7,'DONE': 8,'ERROR_CLIP': 10001,'ERROR_UPLOAD_FILE': 10002,'ERROR_UPLOAD_INFO': 10003,'ERROR_UPLOAD_LOGIN': 10004,'ERROR_NO_BODY': 10005,'CEIL_OF_REQUEST': {'COUNT': 10}});
    YNote.Common.extend(YNote.Clipper.prototype, {'close': function() {
            ...
            if (node.nodeType == 3) {
                ...
                if (node === config.doc.mainContent && config.doc.contentType !== '3') {
                    var temp = (new Date().getTime() / 100000) + '';
                    str += temp;
            ...
            + '" name="' + config.doc.contentType + 'ContentFrame' 
            + '" onload="yApp.frameHandler(event);" style="border:0px" scrolling ="no"></iframe>';
            ...
            YNote.Common.extend(form, {'id': config.clipperDomPrefix + 'ContentForm','name': config.clipperDomPrefix + 'ContentForm','action': config.clipperBaseURL + config.clipperUploadApp,'target': config.doc.contentType + 'ContentFrame','enctype': 'multipart/form-data','encoding': 'multipart/form-data','method': 'POST'});
            YNote.Common.extend(formContainer.style, {'display': 'none'});
            var formInnerHTML = '';
    ...
    YNote.Common.extend(YNote.Selection.prototype, {'getSelection': function() {
            var win = window;
                    ...
                    case 3:
                        ...
                        if (nodeLength < rangeLength) {
                            ...
                            if (direction == 1)
                                ...
                                return {'el': node,'offset': nodeLength - rangeLength};
                        }
                        break;
                    case 1:
    ...
    YNote.Common.extend(YNote.ClipperManager.prototype, {'run': function() {
    ...
    YNote.App.prototype = {'crossDomain': (function() {
            var croDomain = {};
            var interval_id, last_hash, cache_bust = 1, rm_callback, window = this, FALSE = !1, postMessage = 'postMessage', addEventListener = 'addEventListener', p_receiveMessage, has_postMessage = window[postMessage];
        ...
        var loopFunc = function() {
            ...
            if ((document.readyState == 'complete' || document.readyState == 'loaded' || document.readyState == 'interactive') && document.body) {
                ...
                yApp.crossDomain.receiveMessage(function(e) {
                    ...
                    } else if (e.data === 'resize_fullpage' || e.data === 'resize_login') {
                        var height = 248;
                        if (e.data === 'resize_fullpage') {
                            height = 285;
                        }
