...
def _updateCache(request_headers, response_headers, content, cache, cachekey):
    if cachekey:
        ...
        if cc.has_key('no-store') or cc_response.has_key('no-store'):
            ...
            # as_string() will split line longer than 48 into several lines, 
            # so my user-agent(very long) can not remain as it was after been stored in the cache
            # which will lead to not using cache just because the cached one isnot the same as the UA from header
            # see cached file format for convincing
            #####i donot know why sometimes info returns Unicode, which will cause
            #####text = "".join([status_header, header_str, content]) to raise a
            #####Unicode decode error, so I have to convert it to str
